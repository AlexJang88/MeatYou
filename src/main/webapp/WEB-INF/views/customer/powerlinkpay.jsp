  ...
  <h3 align="center">비용은 클릭당 100원으로 진행됩니다!</h3> 
	<table border="1" width="1300" cellpadding="0" cellspacing="0" align="center">				
		<tr height="30"> 
            ...
            <input type="hidden" id="quantity" name="quantity" value="${co_quantity}">	           	
			...
			<tr align="center">
				...
				<td valign="top">
				    <c:choose>
						<c:when test="${clickpay == 10000}"> ${co_quantity} 회 </c:when>
						<c:when test="${clickpay == 30000}"> ${co_quantity} 회</c:when>
						<c:when test="${clickpay == 50000}"> ${co_quantity} 회</c:when>
						<c:when test="${clickpay == 100000}">${co_quantity} 회</c:when>											       
						<c:when test="${clickpay == 200000}">${co_quantity} 회</c:when>											       
					</c:choose>
				</td>
				<td>${clickpay}원</td>	
		       	<td>		   						
